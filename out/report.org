...
#+begin_src R :results graphics file :file read_length_log.png
  ...
  abline(v = c(1900, 5000))
  text(1900, max(reads.all.length), '18S', srt=90, adj = c(.5, 0))
  text(5000, max(reads.all.length), '28S', srt=90, adj = c(.5, 0))
...
#+end_src
#+CAPTION: The number of the reads (y-axis) by the lengths (x-axis; log-scale) and the mapping qualities.
  ...
  legend(3000, max(reads.all.length), lty=1, col=tmp.cols, bg = 'white',
         legend=paste(c('All', 'Q≥0', 'Q≥7', 'Q≥10'),
                      c(sum(reads.all.length),
                        ...
                        sum(reads.mapq10.length)), sep='; '))
#+end_src
#+CAPTION: The number of the reads (y-axis) by the lengths (x-axis; linear-scale) and the mapping qualities.
...
#+begin_src R :results graphics file :file read_chr.png
  tmp.reads <-
    table(scanBam('raw.bam',
                  param = ScanBamParam(
                    ...
                    flag = scanBamFlag(isUnmappedQuery = F)))[[1]][[1]])
  tmp.chromosomes <- ifelse(substr(names(tmp.reads), 1, 3) == 'chr', names(tmp.reads), 'spikein')
  ...
  dev.off()
#+end_src
#+CAPTION: The number of the reads (x-axis) by the origins, either the chromosomes or the spikein (y-axis).
#+LABEL: fig:read_chr
#+RESULT: fig:read_chr
